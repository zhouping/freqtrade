...
# ⚠️ 注意：脚本输出的文件名和位置需要手动调整！见下方「⚠️ 数据目录结构陷阱」
```

**⚠️ 数据目录结构陷阱（关键！）**

freqtrade 对现货和期货数据的目录结构处理不同：
...
**错误表现：**
- 如果现货数据放在 `--datadir/spot/` 子目录下，回测会报 `No history found`
- 如果期货数据放在 `--datadir/` 根目录下，回测也会找不到

**正确操作（现货）：**
```bash
# 1. 下载数据到临时目录
# 2. 将文件复制到 datadir 根目录，并重命名为 freqtrade 标准格式
cd ~/Project/freqtrade/user_data/data/Testnet/spot
cp BTCUSDT_5m_2025.feather ../BTC_USDT-5m.feather
# 3. 验证文件格式（date 列必须是 int64）
python3 -c "import pandas as pd; df=pd.read_feather('../BTC_USDT-5m.feather'); print('date dtype:', df['date'].dtype)"
```

**⚠️ Feather date 列必须是数值型毫秒时间戳（关键陷阱！）**

**问题：** 手动创建的 feather 文件，如果 `date` 列是 pandas datetime 对象，会导致 "No history found" 错误。

**错误表现：**
```python
# ❌ 错误：date 是 datetime64[ns]
df['date'] = pd.to_datetime('2025-01-01 00:10:00')  # dtype: datetime64[ns]
df.to_feather('file.feather')
# ✅ 正确：date 必须是数值型毫秒时间戳 (int64)
...
print('date dtype:', df['date'].dtype)  # 必须是 int64
```

**已下载数据（2026-04-22 确认）：**

现货 (`user_data/data/Testnet/`)：
| 间隔 | 文件名 | 行数 | 时间范围 |
|------|--------|--------|----------|
| 5m | `BTC_USDT-5m.feather` | 105,107 | 2025-01 ~ 2025-12 |
...
> （详细内容已移到上方「⚠️ Feather date 列必须是数值型毫秒时间戳」章节）
...
```
{交易对}-{时间周期}.feather              # 现货 feather（直接放在 datadir/ 下）
{交易对}-{时间周期}-futures.feather     # 期货 feather（放在 datadir/futures/ 下）
```

| 数据源 | 文件名示例 | 目录 |
|--------|-----------|------|
| **现货 feather** | `BTC_USDT-5m.feather` | `user_data/data/Testnet/` |
| **期货 feather** | `BTC_USDT-5m-futures.feather` | `user_data/data/Testnet/futures/` |
| 永续合约（Gate实时） | `BTC_USDT_USDT-5m-futures.json.gz` | `user_data/data/gate/futures/` |
| HTX feather | `BTC_USDT-5m.feather` | `user_data/data/htx/` |

**⚠️ 现货 feather 的目录陷阱：**
- ❌ `user_data/data/Testnet/spot/BTC_USDT-5m.feather` — freqtrade 找不到
- ✅ `user_data/data/Testnet/BTC_USDT-5m.feather` — 现货 feather 直接在 datadir 下

**⚠️ 期货 feather 的目录陷阱：**
- ❌ `user_data/data/Testnet/BTC_USDT-5m-futures.feather` — freqtrade 找不到
- ✅ `user_data/data/Testnet/futures/BTC_USDT-5m-futures.feather` — 期货 feather 在 futures/ 子目录下
