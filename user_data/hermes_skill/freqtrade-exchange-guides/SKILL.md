...
|| **Binance** | 无限制 | 否（公开数据） | **需要代理** |
|| **OKX** | 无限制 | 否 | 需要代理 |
|| **HTX** | 无限制 | 否 | 支持2025年数据 |
|| **Binance Vision S3** | 无限制 | 否 | **最推荐**，直接从AWS下载（不需要代理） |

**核心原则：**
- **本地历史K线**：必须用主网下载的数据（配置 `aiohttp_trust_env: true` + 代理）
- **元数据（交易对信息）**：从 Binance 主网获取
- **禁止混用**：现货模式 + 期货数据是错误的！
- **不需要 Testnet**：直接使用 Binance 主网即可
...
| 间隔 | 文件名 | 时间范围 |
|------|--------|----------|
| 5m | `BTCUSDT_USDT-5m-futures.feather` | 2025-01 ~ 2025-12 |

**数据位置：**
- 现货：`user_data/data/`（根目录）
- 期货：`user_data/data/futures/`
...
freqtrade download-data -c config.json --pairs BTC/USDT --timerange 20250101-20251231 -t 5m
```
