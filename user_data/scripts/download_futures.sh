...
BASE_URL="https://data.binance.vision/?prefix=data/futures/um/monthly/klines/BTCUSDT"
...
for TF in "${TIMEFRAMES[@]}"; do
    for YEAR in "${YEARS[@]}"; do
        ...
        if [ -f "/tmp/${FILE}" ]; then
            ...
            # 移动文件，命名格式：BTC_USDT_USDT-{timeframe}-futures.feather
...
feather.write_feather(df, "${DATA_DIR}/BTC_USDT_USDT-${TF}-futures.feather")
EOF
                    rm -f "$f"
                fi
            done
            ...
            echo "完成: BTC_USDT_USDT-${TF}-futures.feather"
        else
            echo "跳过: ${FILE} 不存在"
        fi
    done
